...
win32-g++ {
    ...
    COPY_CMD = cp
    # sh: 文件不存在时跳过
    COPY_QM = -cp ../../translations/*.qm
} else:win32-msvc* {
    MKDIR_CMD = mkdir
    MKDIR_ERR = 2>nul || rem
    COPY_CMD = copy /y
    # cmd: 文件不存在时跳过
    COPY_QM = if exist ..\\..\\translations\\*.qm copy /y ..\\..\\translations\\*.qm
} else {
    MKDIR_CMD = mkdir -p
    COPY_CMD = cp
    COPY_QM = -cp ../../translations/*.qm
...
# 主题 & 翻译
QMAKE_POST_LINK += $$COPY_CMD ../../themes/*.qss \"$$shell_path($${DESTDIR}/themes)\" $$escape_expand(\\n\\t)
